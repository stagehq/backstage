export function generateGradient(name: string): string {
  const colors = ["red", "orange", "yellow", "green", "blue", "purple"]; // Define an array of colors for the gradient

  const hue =
    name.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) %
    colors.length; // Generate hue based on character codes of name
