    ...
    if (bio !== site?.bio || tagline != site?.tagline) {
      if (site?.subdomain && site?.bio && site?.tagline) {
        ...
        if (!response.data?.updateSiteHeader) {
          console.log("We're sorry! Something went wrong while updating the header.");
          toast.error("We're sorry! Something went wrong while updating the header.");
        }else{
